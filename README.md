...
本仓库托管 wUwproject 技能合集，由 git-sync 自动同步维护。码云（Gitee）和 GitHub 双平台同步。

> 历史提交保留于永久存档仓库 workbuddy-skills（skills/ 目录）：Gitee https://gitee.com/wUwproject/workbuddy-skills | GitHub https://github.com/Ldxs001/workbuddy-skills
...
| `color-toolkit-turn` | 专业颜色工具集，支持颜色编码转换、对比度计算、智能颜色推荐、HTML预览生成。适用于UI设计、无障碍开发、配色方案生成等场景。 |
...
| `git-sync` | 全平台发布工具。多仓库模型（skill→maby_skills, agent→maby_agent）、LLM 文件筛除不卡死、路径由 manifest 统一管理、Release 只打 tag 不传 ZIP、PyPI 版本自动归一化 PEP 440、敏感信息强制脱敏不可跳过。 |
...
```bash
cd ~/.workbuddy/skills
git clone https://gitee.com/wUwproject/maby_skills.git temp-skills
cp -r temp-skills/* .
rm -rf temp-skills
```
...
```bash
cd ~/.workbuddy/skills/
git clone https://github.com/Ldxs001/maby_skills.git temp-skills
cp -r temp-skills/* .
rm -rf temp-skills
```
